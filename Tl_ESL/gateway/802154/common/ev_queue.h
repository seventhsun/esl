 ...
 */
typedef struct {
    struct queue_item_t *next;        //!<  Pointer to the next element
    unsigned char data[1];                     //!<  Pointer to the data
} queue_item_t;
...
ev_queue_sts_t ev_queue_init( ev_queue_t *q, ev_priFunc_t priFunc);
 ...
 /**
   ...
   * @return      Status
   */
ev_queue_sts_t ev_queue_push(ev_queue_t *q, unsigned char *payload);
...
/**
  ...
  */
unsigned char *ev_queue_pop(ev_queue_t *q);
   ...
   * @return      Status
   */
ev_queue_sts_t ev_queue_delete(ev_queue_t *q, unsigned char *payload);
